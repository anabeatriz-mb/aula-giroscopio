...
export default function App() {
  ...
  const formatAxis = (value: number) =>
    Object.is(value, -0) ? "0.0" : value.toFixed(1);
  ...
  return (
    <View style={styles.container}>
      ...
      */}
      <Text style={styles.text}>x: {formatAxis(x)}</Text>
      <Text style={styles.text}>y: {formatAxis(y)}</Text>
      <Text style={styles.text}>z: {formatAxis(z)}</Text>
    </View>
  );
}
